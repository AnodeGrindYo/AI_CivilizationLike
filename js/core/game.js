class Game {
    ...
    addPlayer(player) {
        ...
        if (startPos) {
            ...
            // Add initial units around capital
            this.addInitialUnits(player, startPos);
        }
    ...
    addInitialUnits(player, startPos) {
        // Add some initial units to create more action
        // Warrior
        const warrior = new Unit({
            type: 'warrior',
            owner: player,
            location: {...startPos, x: startPos.x + 1}
        });
        player.units.push(warrior);
        this.map.placeUnit(warrior);
        
        // Settler
        const settler = new Unit({
            type: 'settler',
            owner: player,
            location: {...startPos, y: startPos.y + 1}
        });
        player.units.push(settler);
        this.map.placeUnit(settler);
    }
